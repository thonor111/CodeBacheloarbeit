...
def make_model(features, counties, params):
    ...
    phi_0 = params[0]['phi'][0]
    phi_1 = params[0]['phi'][1]
    phi_2 = params[0]['phi'][2]
    phi_0_neighbour = params[1]['phi'][0]
    phi_1_neighbour = params[1]['phi'][1]
    phi_2_neighbour = params[1]['phi'][2]


    with pm.Model() as ar_model:

        phi_self = pm.Uniform('phi_self', lower=0.0, upper=1.0)
        # phi_0 = pm.Constant('phi_0', params[0]['phi'][0])
        # phi_1 = pm.Constant('phi_1', params[0]['phi'][1])
        # phi_2 = pm.Constant('phi_2', params[0]['phi'][2])
        # phi_0_neighbour = pm.Constant('phi_0_neighbour', params[1]['phi'][0])
        # phi_1_neighbour = pm.Constant('phi_1_neighbour', params[1]['phi'][1])
        # phi_2_neighbour = pm.Constant('phi_2_neighbour', params[1]['phi'][2])

        # phi_0 = pm.Normal('phi_0', 0.0, 1.0)
        # phi_1 = pm.Normal('phi_1', 0.0, 1.0)
        # phi_2 = pm.Normal('phi_2', 0.0, 1.0)
        # phi_0_neighbour = pm.Normal('phi_0_neighbour', 0.0, 1.0)
        # phi_1_neighbour = pm.Normal('phi_1_neighbour', 0.0, 1.0)
        # phi_2_neighbour = pm.Normal('phi_2_neighbour', 0.0, 1.0)

        prediction_self = phi_0 * data.to_numpy() + phi_1 * data_weekly_mean.to_numpy() + phi_2 * data_weekly_mean_squared.to_numpy()

        prediction_neighbour = phi_0_neighbour * interaction_data.to_numpy() + phi_1_neighbour * interaction_data_weekly_mean.to_numpy() + phi_2_neighbour * interaction_data_weekly_mean_squared.to_numpy()
        ...
        # mu_log_neighbour = phi_self * prediction_self + (1 - prediction_self) * prediction_neighbour
        # # mu_log = prediction_self
        #
        # mu_neighbour = pm.Deterministic(
        #     "mu",
        #     mu_log,
        # )
        #
        # Y_obs_neighbour = pm.Poisson(
        #     mu=mu,
        #     observed=target,
        #     name='Y_obs'
        # )
...
def sample_predictions(
    ...
    phi_0 = parameters_models[0]['phi'][0]
    phi_1 = parameters_models[0]['phi'][1]
    phi_2 = parameters_models[0]['phi'][2]
    phi_0_neighbour = parameters_models[1]['phi'][0]
    phi_1_neighbour = parameters_models[1]['phi'][1]
    phi_2_neighbour = parameters_models[1]['phi'][2]
    ...
    num_parameter_samples = phi_self.shape[0]
